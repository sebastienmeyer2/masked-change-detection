...
class PadIm2Video(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[self.time_dim] % 2 == 1:
            nb_frames = x.shape[self.time_dim]
            ...
            x = x.repeat_interleave(repeat_tensor, dim=self.time_dim)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        npatch_per_img = x.size(1) - self.first_patch_idx
        npatch_per_img = np.prod(self.patches_layout[1:])
        ...
        nb_patch_complete = x.shape[1] // pos_embed.shape[1]
        pos_embed_complete = einops.repeat(
            pos_embed, "b n e -> b (repeat n) e", repeat=nb_patch_complete
        )

        left_frames = x.shape[1] - pos_embed.shape[1] * nb_patch_complete
        pos_embed_incomplete = pos_embed[:, :left_frames, ...]

        pos_embed = torch.cat((pos_embed_complete, pos_embed_incomplete), dim=1)
        x = x + pos_embed
...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x, npatch_to_keep, mask):
        ...
        npatch_per_img = np.prod(self.patch_embed.patches_layout[1:])
        ...
        # Create a cycle of positional embedding every "pos_embed" length to be able to feed the
        # model with sequences of arbitrary number of frames
        nb_patch_complete = x.shape[1] // pos_embed.shape[1]
        pos_embed_complete = einops.repeat(
            pos_embed, "b n e -> b (repeat n) e", repeat=nb_patch_complete
        )
    ...
    def interpolate_pos_encoding(
        ...
        if input_shape is None or patches_layout[0] == 1:
            ...
            for i in range(num_frames):
                # interpolate embedding for ith frame
                pos_embed[0, i] = cls.interpolate_pos_encoding_2d(
                    npatch_per_img, pos_embed[0, i, ...].unsqueeze(0)
                )
            pos_embed = pos_embed.view(1, num_frames * num_spatial_tokens, -1)
        else:
